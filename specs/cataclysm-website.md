...
- Deploying a Worker that declares an `[[analytics_engine_datasets]]` binding requires Analytics Engine to be switched on once for the account first. Measured 2026-09-02: `wrangler deploy` uploads the script and the Cloudflare API rejects it with `code: 10089`, "You need to enable Analytics Engine", pointing at `https://dash.cloudflare.com/<account_id>/workers/analytics-engine`. There is no wrangler subcommand and no documented API for the switch, so it is a one-time dashboard click by the account owner. `wrangler deploy --dry-run` does not catch it: the binding check passes locally while the real deploy fails, and the failed deploy leaves nothing behind (the route still served the origin 404 afterwards).
...
- That Analytics Engine never samples at this volume. Verify by asserting `_sample_interval = 1` in `stats.sh` and failing loudly when it is not. Still open as of 2026-09-02: unchecked on two counts, the Worker is not deployed so neither dataset has ever been written, and keychain item `claude-local-cloudflare` does not exist so `stats.sh` has no token. The assert is implemented and covered by `worker/stats.sh --dry-run`.
- That the GitHub API's unauthenticated per-IP budget is reachable from a Cloudflare Worker. Verify at w.1 by hitting the deployed route repeatedly and watching for 403s. Still open as of 2026-09-02: unchecked, there is no deployed route to hit. The 10-minute module-scope cache that is meant to keep the budget out of reach is implemented and covered by unit tests.
...
| # | Item | State | Notes |
| --- | --- | --- | --- |
| w.0 | Cloudflare account setup: `npx wrangler login` for deploys (renewed 2026-09-02), account id `9bdc7b059bbd6f7e6204d2d23e03e025`; an API token with Account Analytics Read in keychain `claude-local-cloudflare` for `stats.sh`; Analytics Engine switched on for the account | blocked 2026-09-02, two user actions outstanding | User present, both steps are human-only. (1) Analytics Engine is off: `wrangler deploy` fails with `code: 10089` and no wrangler command or documented API turns it on, so the owner must click it on at `https://dash.cloudflare.com/9bdc7b059bbd6f7e6204d2d23e03e025/workers/analytics-engine`; until then the Worker cannot deploy and w.1's live checks cannot run. (2) The analytics token does not exist yet, and the login session's OAuth scopes cannot query the Analytics SQL API, so `stats.sh` waits on it. No dataset creation step: datasets appear on first write |

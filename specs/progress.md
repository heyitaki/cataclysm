...
Handoff (2026-08-31): every automatable item below (0.1 through 3b.3) is delegated to an unattended ralphex run driven by `docs/plans/cataclysm-distribution.md`, which executes on its own branch. Do not start items from this table while that run is live; when it finishes, reconcile this table against the plan's ticked checkboxes and the run branch. The `needs-user` rows stay the user's.

| # | Item | Spec | State |
| --- | --- | --- | --- |
| 0.1 | Split `main.swift` into modules (`Jail.swift`, `TapHost.swift`), no behavior change, still CLI under Hammerspoon, binary still `mousejail` | pointer-and-scroll phase 0 | not-started(unblocked: mousejail-73's session ended after landing its jail work as a2885b3 and 51edbc0; its standing guidance still binds — rounded-rect Clamp type, --corner-radius flag, and the unconditional warp in the callback are load-bearing, move code without changing behavior) |
...
| 1.1 | Pointer acceleration (`PointerAccel.swift`) behind a CLI flag, bridging header, -1 guard, 196608 bootstrap, adopt rule, 5s reassert, wake reassert | pointer-and-scroll phase 1 | done-pending-wiring(module by mousejail-ce, verified by mousejail-f4, then hardened in a second mousejail-ce review round: restore keeps its claim when the restoring write fails so a retry can still put the original back; reassert re-takes an unreadable property instead of going inert; ~15s of consecutive failed reads surfaces as unhealthy via onWriteHealthChange, and a read of -1 clears a stuck failure flag; restore no longer fires the health callback during teardown/deinit; disable() asserts the main queue. Public API unchanged; compiles clean with and without the 13.0 target. Deliberate deviation: bootstrap honors a non-positive com.apple.mouse.scaling by restoring nothing instead of forcing 196608. CLI flag wiring lands with 0.1; owner's exit paths must call restore() — deinit does not run for globals at exit) |

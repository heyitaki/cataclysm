...
| r.8 | Rename the GitHub repo `mousejail` to `cataclysm` | Done 2026-09-01: `heyitaki/cataclysm`, old URL redirects, origin updated. Local checkout moved to `~/code/cataclysm` and re-registered in Orca; the stale `mousejail` repo card needs removing from the Orca UI (no CLI removal) |

...
The row replaces the interim "Check for updates…" item (`CataclysmApp.swift:1236`), which website plan w.5a repointed at `akshath.me/cataclysm` on 2026-09-02; u.6 supersedes it.
...
| u.2 | Makefile: pinned Sparkle fetch with digest check, `-F`/`-framework`/rpath on both slices and `typecheck`, framework embedded with XPC services removed and the four-step re-sign, `--options runtime` dropped, `codesign --verify --strict --deep` | not started | Blocked on nothing. Website plan w.2 (2026-09-02) already rewrote the Makefile's signing comment to say releases are never notarized; drop the flag and the comment's "ready for a notarizing identity" sentence together |
| u.3 | `make dmg` also emits `Cataclysm-<version>.zip` and a byte-identical `Cataclysm.dmg`; new `make appcast` target; README release section lists the four assets and the two commands | partial 2026-09-02 | The zip (`ditto -c -k --keepParent`, app inside passes `codesign --verify --strict` after extraction) and the version-stable `Cataclysm.dmg` (`cmp`-identical) land with website plan w.2, ahead of u.2; `make dmg` also stopped signing the image. `make appcast` and the README release section are still outstanding. `make appcast` stays out of `make dmg` because it needs the keychain |
...
| u.6 | Panel: state-driven row replacing the interim website link, `errorRow` under it for manual-pass errors only, the disabled "cannot verify updates" row when the updater failed to start | not started | Blocked on u.5. Supersedes website plan w.5a |

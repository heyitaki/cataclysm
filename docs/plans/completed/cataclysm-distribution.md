...
- Verify crash recovery actually runs after the first real launch: the smoke gate measured `SMAppService.register()` succeeding with the self-signed identity while launchd never spawned the watcher (`needs LWCR update`, no pid after 10s), so a runtime registration that reads `.enabled` may still be hollow. The runtime now runs the gate's spawn check after `register()` succeeds and falls back to the legacy job on non-spawn (review fix, 2026-09-01). The system log explains the non-spawn: launchd spawns the SMAppService job, then fails it with `Unable to get updated LWCR ... Invalid argument` because BTM ignores the plist's bundle identifiers for an executable without a Team ID, and throttles respawns. A second measured effect: BTM keys launch items by label, so writing the legacy plist under the shared label re-enables the SMAppService agent's BTM record and `smd` re-submits that job once the label is free (the gate now checks for and undoes this in its last step). After the first GUI launch, check `launchctl print gui/$UID/io.github.heyitaki.cataclysm.watch` shows a pid, note which mechanism holds the label (`managed_by = com.apple.xpc.ServiceManagement` versus the `~/Library/LaunchAgents` path), and confirm the watcher survives a logout/login with both the legacy plist and the re-enabled BTM record competing for the label.
